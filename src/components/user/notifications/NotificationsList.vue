                ...
                <div class="element-box-tp">
                    <div class="table-responsive">
                        <table class="table table-padded">
                            ...
                            <tbody>
                            <tr v-for="message in messages.data"  @click.prevent="readNotification(message.id)">
                                <td v-if="message.type == 1">{{ message.user.email }}</td>
                                <td v-else>Geral (Todos)</td>
                                <td>{{message.subject}}</td>
                                <td v-html="message.content.substring(0, 50)"> ...</td>
                                <td v-if="message.status">Lida</td>
                                <td v-if="!message.status">Não Lida</td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
